...
public class EmployeeService implements EService<Employee>{
    ...
    public void changeStatusE(int index) {
        if (!employeeList.get(index).isStatus()) {
            employeeList.get(index).setStatus(true);
            EmployeeDTO.writeFile(employeeList);
        }
        System.out.println("...Điểm danh thành công: '"+employeeList.get(index).getName()+"'");
    }
    ...
    public void calculateSalary(String type) {
        ...
        System.out.println("id, tên, số ngày công, lương tháng");
        for (Employee e:employeeList){
